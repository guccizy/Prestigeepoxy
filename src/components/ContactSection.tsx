...
const ContactSection = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    console.log(t('contact_section.form_sending_message'), formData);

    try {
      const response = await fetch('http://localhost:5000/api/contact', { // Nouvel endpoint pour le formulaire de contact
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        alert(data.message || t('contact_section.success_message'));
        setFormData({
          name: '',
          email: '',
          phone: '',
        ...
        alert(data.message || t('contact_section.error_message'));
      }
    } catch (error) {
      console.error(t('contact_section.form_send_error'), error);
      alert(t('contact_section.alert_error_message'));
    }
  };
  ...
  const contactMethods = [
    ...
    // Removed Showroom section as per user request
    // {
    //   icon: ShoppingBag,
    //   title: 'Showroom',
    //   subtitle: 'Découvrez nos finitions',
    //   value: '1234 Rue Saint-Denis, Montréal',
    //   bgColor: 'bg-elegant-gold/10'
    // }
  ];
